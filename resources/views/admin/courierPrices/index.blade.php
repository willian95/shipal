                    ...
                    <div class="card card-custom gutter-b">
                        <div class="card-header flex-wrap py-3">
                            <div class="card-title">
                                <h3 class="card-label">Tarifas couriers
                            </div>
                            ...
                            <table class="table table-bordered table-checkable" id="kt_datatable" style="margin-top: 15px;">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Peso</th>
                                        <th>Zona</th>
                                        <th>Tarifa Courier</th>
                                        <th>Tarifa Shipal</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="(courierPrice, index) in courierPrices">
                                        
                                        <td>@{{ courierPrice.courier.name }}</td>
                                        <td>
                                            @{{ courierPrice.weight.weight }}
                                        </td>
                                        <td>
                                            @{{ courierPrice.zone_id }}
                                        </td>
                                        <td>
                                            @{{ courierPrice.courier_price }}
                                        </td>
                                        <td>
                                            @{{ courierPrice.shipal_price }}
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
        ...
        const app = new Vue({
            ...
            methods:{
                ...
                store(){
                    ...
                    this.loading = true
                    axios.post("{{ url('/admin/courier-prices/store') }}", {file: this.file, courierId: this.courierId})
                    .then(res => {
                        this.loading = false
                ...
                fetch(page = 1){
                    ...
                    this.page = page
                    axios.get("{{ url('/admin/courier-prices/fetch') }}"+"/"+this.page)
                    .then(res => {

                        if(res.data.success == true){

                            this.courierPrices = res.data.courierPrices
                            this.pages = Math.ceil(res.data.courierPricesCount / res.data.dataAmount)
